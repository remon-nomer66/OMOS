...
int tableReg(PGconn *__con, int __soc, int *__u_info, int *__s_info){
    ...
    while(1){
        ...
        if(PQntuples(res) == 0 && cnt <= 99 && cnt != 0){ //入力された値が２桁の数字だった場合
            ...
            if(resultRows == 1){
                ...
                resultRows = PQntuples(res);
                if(resultRows != 1){
                    sprintf(sendBuf, "テーブル番号%dを登録できませんでした%s", cnt, ENTER); //送信データ作成
                }else{

...
int history(pthread_t selfId, PGconn *con, int soc, char *recvBuf, char *sendBuf, int *s_info){
    ...
    int num_price[100][4];  //menu_price_tのレコードを格納する配列
    ...
    for(int i = 0; i < PQntuples(res); i++){
        if(PQgetisnull(res, i, 2) == 0){ // kitchenflagが"未"
            sprintf(sendBuf, "%s %s %s%s", PQgetvalue(res, i, 0), PQgetvalue(res, i, 1), "未", ENTER);
    ...
    for(int i = 0; i < PQntuples(res); i++){
        if(PQgetisnull(res, i, 2) == 0){ // kitchenflagが"済み"
            sprintf(sendBuf, "%s %s %s%s", PQgetvalue(res, i, 0), PQgetvalue(res, i, 1), "済", ENTER);
            sendLen = strlen(sendBuf);
            send(soc, sendBuf, sendLen, 0);
            printf("[C_THREAD %ld] SEND=> %s\n", selfId, sendBuf);
        }
    ...
    //顧客に合計金額を表示する
    sprintf(sendBuf, "合計金額: %d%s%s", totalAmount, ENTER, DATA_END);
    sendLen = strlen(sendBuf);
    send(soc, sendBuf, sendLen, 0);
    printf("[C_THREAD %ld] SEND=> %s\n", selfId, sendBuf);

...
int test_func(PGconn *__con, int __soc, int __auth){
  ...
  if(tmp == 1){
    ...
    while(1){
      ...
      if(recvBuf_tmp >= 0000 && recvBuf_tmp <= 2359){
	//取得した対象の時間を格納
	//char start_target[6];
  ...
  if(tmp == 1){
    ...
    char sql[BUFSIZE];
    sprintf(sql, "SELECT * FROM region_t WHERE area_id = %s", recvBuf);
    //PGresult *res = PQexec(__con, sql);
    if(PQntuples(res) == 1){
      //入力された地域番号を格納
      //char area_id[3];
  ...
  if(tmp == 1){
    ...
    char sql[BUFSIZE];
    sprintf(sql, "SELECT * FROM summary_t WHERE menu_id = %s", recvBuf);
    //PGresult *res = PQexec(__con, sql);
    if(PQntuples(res) == 1){
      //入力された商品番号を格納
      //char product_id[5];
  ...
  "SELECT summary.*, price_change_t.price, user_authority_t.auth FROM summary "
    ...
    //CORの地域情報を取得と絞り込み
    "WHERE summary.order_date >= %s AND summary.order_date <= %s AND user_authority_t.auth = %d", start, end, __auth;

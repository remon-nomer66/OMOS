        ...
        while (1){
            ...
            if (strcmp(recvBuf, "y") == 0){
                ...
                if (strcmp(recvBuf, "food") == 0){
                    while (1){
                        ...
                        if (strcmp(recvBuf, "end") == 0){
                         break;
                            ...
                            //4文字以外の場合はエラーを返す
                            if (strlen(recvBuf) != 4){
                                sprintf(sendBuf, "%s。%d%s", ER_STAT, E_CODE_2001, ENTER); //送信データ作成
                                sendLen = strlen(sendBuf); //送信データ長
                                send(soc, sendBuf, sendLen, 0); //送信
                                check = 1;
                            }
                            ...
                            if (strlen(recvBuf) == 4 && !isdigit(recvBuf[0]) || !isdigit(recvBuf[1]) || !isdigit(recvBuf[2]) || !isdigit(recvBuf[3])){
                                sprintf(sendBuf, "%s。%d%s", ER_STAT, E_CODE_2002, ENTER); //送信データ作成
                                sendLen = strlen(sendBuf); //送信データ長
                                send(soc, sendBuf, sendLen, 0); //送信
                                check = 1;
                            }
                            ...
                            if (PQresultStatus(res) == PGRES_TUPLES_OK){
                                if (PQntuples(res) != 0){
                                    sprintf(sendBuf, "%s。%d%s", ER_STAT, E_CODE_2003, ENTER); //送信データ作成
                                    sendLen = strlen(sendBuf); //送信データ長
                                    send(soc, sendBuf, sendLen, 0); //送信
                                    check = 1;
                                }
                            ...
                            if (check != 1){
                                ...
                                // もしうまくいかなければエラーを表示する
                                if (PQresultStatus(res) != PGRES_TUPLES_OK){
                                    sprintf(sendBuf, "%s。%d%s", ER_STAT, E_CODE_2004, ENTER); //送信データ作成
                                    sendLen = strlen(sendBuf); //送信データ長
                                    send(soc, sendBuf, sendLen, 0); //送信
                                    check = 1;
                                }
                                ...
                                if (PQntuples(res) == 0){
                                    sprintf(sendBuf, "%s。%d%s", ER_STAT, E_CODE_2005, ENTER); //送信データ作成
                                    sendLen = strlen(sendBuf); //送信データ長
                                    send(soc, sendBuf, sendLen, 0); //送信
                                    check = 1;
                                }
                                ...
                                if (check != 1){
                                    ...
                                    if (PQresultStatus(res) != PGRES_COMMAND_OK){
                                        sprintf(sendBuf, "%s。%d%s", ER_STAT, E_CODE_2006, ENTER); //送信データ作成
                                        sendLen = strlen(sendBuf); //送信データ長
                                        send(soc, sendBuf, sendLen, 0); //送信
                                        check = 1;
                                    }
                    ...
                    sprintf(sendBuf, "%s。%d%s", ER_STAT, E_CODE_2007, ENTER); //送信データ作成
                    sendLen = strlen(sendBuf); //送信データ長
                    send(soc, sendBuf, sendLen, 0); //送信
                }

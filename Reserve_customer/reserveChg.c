...
int reserveChg(PGconn *__con, int __soc, int *__auth){
    ...
    while(1){
        ...
        if(resultRows <= 0){
            sprintf(sendBuf, "予約変更の対象がありません%s", ENTER);
            sendLen = strlen(sendBuf);
            send(__soc, sendBuf, sendLen, 0);
            printf("[C_THREAD %ld] SEND=> %s\n", selfId, sendBuf);
        ...
        sprintf(sendBuf, "変更する予約番号を入力してください%s予約番号 店舗名 予約日 予約時間%s予約削除から抜ける場合は\"END\"と入力してください%s", ENTER, ENTER, ENTER);
        for(i = 0; i < resultRows; i++){
        ...
        while(1){
            ...
            if(recvLen != 0){   //文字が入力されたら
                ...
                if(cnt == 1 && 1 <= param && param <= RSRVMAX){
                    sprintf(sendBuf, "変更する対象は%s%3d %s %s %s%sでよろしいですか？よろしい場合は\"YES\"と入力してください%s", ENTER, &param, reserve_store_name[param], reserve_date[param], reserve_time[param], ENTER, ENTER);
                    send(__soc, sendBuf, sendLen, 0);
                    printf("[C_THREAD %ld] SEND=> %s\n", selfId, sendBuf);

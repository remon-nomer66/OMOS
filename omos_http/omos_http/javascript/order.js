...
document.addEventListener('DOMContentLoaded', function(){
    let url = "http://localhost:10000?orderCheck=1&layer=6";    //localhostは適宜開いているサーバに変更
    ...
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4){
            if(xhr.status === 200){
                ...
                document.getElementById("whole").textContent = '問題が発生しました';
            }
        }else{
...
function tabChg(){
    let url = "http://localhost:10000?orderCheck=1&";   //localhostは適宜開いているサーバに変更
    url += "layer=";
...
document.getElementById('tab').addEventListener('change', tabChg, false);
                ...
                xhr.onreadystatechange = function(){
                    if(xhr.readyState === 4){
                        if(xhr.status === 200){
                            ...
                            let pCheckboxes = document.querySelectorAll('.order-p-r');
                            pCheckboxes.forEach((pCheckbox) => {
                                pCheckbox.remove();
                            });
                            tmp_cnt = 1;
...
function listener(){
    if(this.checked){
        if(tmp_cnt > 5){
            ...
            let trclas = document.createAttribute('class');
            trclas.value = "order-p-r";
            tr.setAttributeNode(trclas);
...
function pClickEvent(e){
    if(!e.target.checked){
        ...
        if(document.getElementById(e.target.name) != null){
            window.alert('同一注文では同じ種類の商品を注文できません');

...
int response_reserve_check_OK(pthread_t selfId, PGconn *con, int soc, char *http_header, char *http_body, int *u_info, int *body_size){
    ...
    if(resultRows == 0){
        ...
        PQexec(con, sql);
        PQclear(res);
        sprintf(http_body, "<input id=\"btn\" type=\"button\" value=\"送信\" onclick=\"btnEvent(event)\" /><h5>現在ご予約はありません</h5>");
    ...
    sprintf(http_body, "<input id=\"btn\" type=\"button\" value=\"送信\" onclick=\"btnEvent(event)\" /><h5>予約一覧<h5><table border=\"1\"><tr><th>店舗名</th><th>日付</th><th>時間</th><th>人数</th>");
    for(i = 0; i < tmp; i++){
    ...
    sprintf(http_body + strlen(http_body), "</table>");

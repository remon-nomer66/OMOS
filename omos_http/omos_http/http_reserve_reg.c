...
int http_reserve_reg(pthread_t selfId, PGconn *con, int soc, int *u_info, char command[][BUFSIZE], char param[][BUFSIZE]){
    char sql[BUFSIZE];
    PGresult *res;
    int resultRows, tmp, flag, i;
    ...
    sprintf(param[3], "%s:%s", r_hour, r_min);
    ...
    for(i = 0; i < tmp; i++){
        ...
        if(resultRows == 0){
            sprintf(sql, "INSERT INTO reserve_t (reserve_no, user_id, people_num, reserve_date, reserve_time, store_id, desk_num) VALUES (nextval('reserve_seq'), %d, %s, '%s', '%s:00', %s, %d)", u_info[0], param[4], param[2], param[3], param[1], a_table[i]);
            res = PQexec(con, sql);
